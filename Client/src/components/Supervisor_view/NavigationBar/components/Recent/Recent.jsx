...
const Recent = () => {
  ...
  const toggleRecent = () => {
    ...
    const fetchData = async()=>{
      try{
        const response = await axios.get("http://localhost:8000/api/v1/user/projects  ", {
        headers: {
    ...
    <div className={styles.dropdown_contrecent}>
      ...
      {isOpen  && (
        <div className={styles.dropdown_contentrecent}>
          {recentProjects ? <>
            {recentProjects.length > 0 ?(<ul className={styles.projectlist2}>
            {recentProjects.map((pro, key)=>(<li>{pro}</li>))}
          </ul>) : "no recent projects"}
          </>: "loading..."}
        </div>
      )}
    </div>
